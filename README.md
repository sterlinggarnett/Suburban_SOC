...
| M16 | Endpoint Onboarding & Threat-Intel Integrity | ⏸️ 7/8 closed, 1 deferred ([milestone](https://github.com/voltron-1/Suburban_SOC/milestone/20), no actionable work left) — threat-intel retraction (#271), zeek/zeek image pin+CVE bump (#293, #364), and 2 detection-gap follow-ups (#358, #361) all shipped; only #265 (client certs for endpoint shippers) remains — PR #537 (2026-09-03) landed its doc-only half (operator docs now disclose the `tls: certificate required` gap), the cert-minting code still needs a live Docker session; the issue was auto-closed by that PR's squash message and reopened the same day |
| M17 | Detection Rule Coverage & Correctness | ⏸️ 31/34 closed (updated 2026-09-03, [milestone](https://github.com/voltron-1/Suburban_SOC/milestone/22)) — a project-board audit found 7 more review-follow-up issues filed without a milestone and retroactively assigned here after the original "6/8" close; 12 more closed 2026-08-29 via PRs #494–#502; #283 fixed and closed 2026-09-03 (PR #535); #333, #415, #421 remain, needing a live Docker/Zeek/ES stack this environment doesn't have |
| M18 | ECS Pipeline & Field-Mapping Integrity | ⏸️ 16/19 closed (updated 2026-09-04, [milestone](https://github.com/voltron-1/Suburban_SOC/milestone/23)) — #403/#390/#396 shipped and closed 2026-09-03 (PRs #530/#531/#532); **#389 fixed and closed 2026-09-04 (PR #546)** — root-caused to Zeek's log writer (`Log::default_max_field_string_bytes`, 4096 upstream, cut marked only by a `log_string_field_truncated` weird), now pinned to 8191 = `dns.answers`' `ignore_above` so every Zeek-logged answer stays indexed and rule-matchable, with the exact-length tag retargeted, four capture-path guards hardened, two SLO metrics added, and 5 pinned-image tests running green in CI's live-fire job; #326 (externally blocked, needs real Windows/PowerShell telemetry accumulating over time), #405 (needs a live-pipeline perf benchmark or a dedicated Sigma-corpus field-resolution design effort) and #545 (raise `dns.answers` `ignore_above` and the Zeek cap together, needs an array-aware byte clamp) remain |
| M19 | SOC Platform Credential & Secret Hygiene | ✅ Complete (7/7 issues, [milestone](https://github.com/voltron-1/Suburban_SOC/milestone/24), closed 2026-08-29) — cleartext passwords in argv fixed, ES role-provisioning sync check added, live self-check for role regressions, CI toolchain pinned, ES network exposure closed |
| M20 | SOAR Response-Path Hardening | ✅ Complete (6/6 issues, [milestone](https://github.com/voltron-1/Suburban_SOC/milestone/25), closed 2026-08-29) — residual hive-mind-broker/#277 hardening (unbounded request_id, no tampering detection, forged-non-200 retry risk) and the autonomous-isolation MAC-gate policy decision all shipped |
| M21 | Zeek Sensor Operational Resilience | ✅ Complete (3/3 issues, [milestone](https://github.com/voltron-1/Suburban_SOC/milestone/26), closed 2026-08-29) — per-source Zeek liveness SLO metric, CAPTURE_IFACE shell-interpolation fix, symlink/ownership hardening on zeek-host-capture.service, intel-refresh.service config/data co-location + CA trust-on-every-use pinning all shipped |
| M22 | Compliance & Documentation Accuracy | ✅ Complete (7/7 issues, [milestone](https://github.com/voltron-1/Suburban_SOC/milestone/27), closed 2026-08-29) — analyst-facing rule text leaking ES-analyzer detail, an SLO coverage-count off-by-one, a Sigma tag-extraction gap, a never-implemented NIST/CIS tag mandate, and the stale README detection-plane summary (external contributor PR [#440](https://github.com/voltron-1/Suburban_SOC/pull/440)) all closed |
| M23 | Suricata Signature Lane: Sensor, Ingest, CI & Ruleset | ⏸️ 1/4 closed ([milestone](https://github.com/voltron-1/Suburban_SOC/milestone/28)) — #445 (detection-as-code CI lane: syntax gate, SID registry, pcap replay harness) closed via PR #507. #443 (sensor) and #444 (eve.json ECS ingest) are structurally complete (PR #506) but stay open: their own "done when" needs a live capture host + real traffic this sandbox doesn't have, same blocker class as M17's #415/#421. #446 (100-rule starter set, landed via PR #507) stands at 80/100 rules enabled with TP/TN pcap fixtures as of PR #534 (2026-09-03); the remaining 20 are 14 needing `detection_filter` tuning against real traffic, 2 unresolved placeholders, 2 DLP rules needing owner sign-off, 2 dead-as-configured |
| M24 | Security Follow-ups | ✅ Complete (5/5 issues, [milestone](https://github.com/voltron-1/Suburban_SOC/milestone/29), closed 2026-08-30 via PR #505) — HA port exposure, soc_admin audit-trail carve-out, Python pin, auditd Linux execve telemetry pipeline, and 10 Collection/Exfiltration/Linux Sigma rules |
| M25 | Repo Audit Remediation | ✅ Complete (17/17 issues, [milestone](https://github.com/voltron-1/Suburban_SOC/milestone/30), closed 2026-09-03), created 2026-08-31, repo-audit round 2, #508–#523) — cert org-name fix, stale doc/status corrections, missing troubleshooting + playbook entries, dead workflow removal, and UIW branding cleanup all closed via PR #525/#526; #510 (Projects v2 board confirmation) and #522 (wiki Watcher-era pages, 6/6 rewritten) closed 2026-09-03 from a local session with an authenticated `gh` (board verified and re-synced: 44 never-added issues added, every closed issue in Done); #509 done 2026-09-03 by owner decision — `enforce_admins` enabled on `main` via the dedicated endpoint, the script's review gate deliberately not applied (single maintainer); its close-out found `setup_branch_protection.sh` itself stale in the weakening direction (5 named checks vs. 9 live, plus the declined review gate; SOP-007 still says re-run it), filed as #539 and closed the same day (script now unions its check list with the live one so re-runs can only tighten, review gate behind `REQUIRE_REVIEW=1`, SOP-007 step corrected), then hardened further 2026-09-04 via PR #543 (whole live policy through the full PUT, checks pinned to the Actions app, read-back verification, 30-case fake-`gh` test) with doc close-out PR #544. The milestone object was closed 2026-09-07 |
| M26 | Sensor Liveness & Monitoring Independence | 🟡 8/9 closed, reopened ( [milestone](https://github.com/voltron-1/Suburban_SOC/milestone/31), created 2026-09-05, last issues closed 2026-09-07) — opened after a five-day Zeek capture blackout that nothing alerted on: a stale `CAPTURE_IFACE=eth6` pin (WSL2 renumbered the NICs) left the unit crash-looping and then wedged at `active (running)` with zero packets, while `slo-metrics.service` — holding the `zeek_ingest_lag` metric that would have caught it — was itself dead `203/EXEC` on a dangling Docker CLI symlink. PR #551 (merged 2026-09-05) added a capture-interface preflight that exits 78 (`EX_CONFIG`) so `RestartPreventExitStatus=78` parks the unit in `failed` instead of hiding behind `active`, with 20 regression tests. **#550 fixed 2026-09-06 via PR #553**: the ES-CA `docker cp` is now best-effort and `timeout`-bounded, `scripts/setup/es_ca_cache.sh` reinstates the last pin-verified CA from `StateDirectory=` when the Docker CLI is unavailable, and a security-auditor + code-reviewer pass added pin checks in both cache verbs, a non-forgiven usage exit status, `TimeoutStartSec=`, and the removal of `RemoveIPC=true`. The purple-team pass on #550 opened five more: **#555/#557/#558 closed 2026-09-06** (PRs #560/#561/#563 — `stack_health.sh` finally installed on a timer, with the two SOC self-health lanes cross-monitoring each other's output index; `.gitattributes` covers `*.service`/`*.timer`; sandboxing parity ported from `intel-refresh.service`). **#549/#554/#556/#562 closed 2026-09-07 via [PR #564](https://github.com/voltron-1/Suburban_SOC/pull/564)**: an output-derived liveness supervisor for `zeek-host-capture.service` (restarts + alerts only when the unit reports `active` *and* `conn.log` has gone stale, never while `failed`, so a genuine config error stays parked and visible); an `OnFailure=` alert dispatcher (`soc-alert-on-failure@.service`, dependent on neither Docker nor Elasticsearch — the two things the units it watches depend on) wired onto `slo-metrics.service` and `zeek-host-capture.service`; the capture host's own `/var/log/syslog` shipped to Elasticsearch plus 2 new Sigma rules covering a persistently-failing SOC unit and an ES CA-fingerprint mismatch (the Linux-side counterpart to the existing Windows-only T1562.001 coverage); and `.gitattributes` extended to the 4 extensionless files (itself included) it didn't previously govern. **PR #564 was structurally validated only** — no live capture host was reachable from that session — so the live-host deploy (`redeploy_systemd_units.sh`, enabling `zeek-capture-liveness.timer`, a Filebeat/Logstash restart for the new syslog input, `deploy_detections.sh` for the 2 new rules) is the remaining step, and `NTFY_TOPIC` provisioning stays deliberately deferred behind the `slo_metrics` credential-resync gate tracked in `planned_execution.md`. **Reopened 2026-09-08** to hold **#567** (`soc_alert_on_failure.sh` has no rate limit of its own: `OnFailure=` activates on every restart cycle, measured, so a unit failing every `RestartSec` sends one push per cycle — ~17,280/day at 5s — and the only cap today lives in a different file's `StartLimitBurst`). The repo-side capture fixes from that same investigation are in PR #566; the live-host deploy remains outstanding |
...
- **Detection framework enrichment (PR #112).** The detection plane spans
  ...
  **122 Sigma rules** (`rules/sigma/`) each carry their own ATT&CK technique tag and

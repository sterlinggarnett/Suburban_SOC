...
| M22 | Compliance & Documentation Accuracy | ⏸️ 5/7 closed, 2 blocked on an external contributor's in-flight PR ([milestone](https://github.com/voltron-1/Suburban_SOC/milestone/27), no actionable work left) — analyst-facing rule text leaking ES-analyzer detail, an SLO coverage-count off-by-one, a Sigma tag-extraction gap, and a never-implemented NIST/CIS tag mandate all closed; #439/#380 (ravindharann's in-flight PR touches the same README.md lines) remain |

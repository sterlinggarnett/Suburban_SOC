...
| `slo-metrics.service` fails with `203/EXEC` and the SLO dashboard freezes — no `soc-slo-metrics` documents written, no breach alert for anything | The unit extracts the ES CA with `ExecStartPre=…/usr/bin/docker cp …`, and `/usr/bin/docker` is a symlink into the `docker-desktop` WSL distro that **dangles whenever Docker Desktop is stopped**. `203/EXEC` is a systemd-level exec failure, so the unit dies before `ExecStart` and computes nothing at all. Because this is the unit that runs `metric_zeek_ingest_lag_seconds()`, one Docker Desktop outage takes down the capture lane **and** the monitoring that watches the capture lane at the same time (#550, the 2026-08-31 → 09-05 blackout) | Fixed: the `docker cp` is now `-`-prefixed so a missing engine degrades the CA refresh instead of killing the run, and `scripts/setup/es_ca_cache.sh` reinstates the last pin-verified CA from `StateDirectory=` (`/var/lib/suburban-soc-slo/ca.crt`) so the degraded run reports the real reason ES is unreachable rather than a misleading TLS CA-bundle error. If you see this on a host that has not picked up the fix, run `bash scripts/setup/redeploy_systemd_units.sh`. Start Docker Desktop to restore full measurement — with the engine down, Elasticsearch is down too, so ES-backed metrics stay unmeasurable by definition |

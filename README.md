...
| M26 | Sensor Liveness & Monitoring Independence | 🟡 In progress (4/8 issues, [milestone](https://github.com/voltron-1/Suburban_SOC/milestone/31), created 2026-09-05) — opened after a five-day Zeek capture blackout that nothing alerted on: a stale `CAPTURE_IFACE=eth6` pin (WSL2 renumbered the NICs) left the unit crash-looping and then wedged at `active (running)` with zero packets, while `slo-metrics.service` — holding the `zeek_ingest_lag` metric that would have caught it — was itself dead `203/EXEC` on a dangling Docker CLI symlink. PR #551 (merged 2026-09-05) added a capture-interface preflight that exits 78 (`EX_CONFIG`) so `RestartPreventExitStatus=78` parks the unit in `failed` instead of hiding behind `active`, with 20 regression tests. **#550 fixed 2026-09-06 via PR #553**: the ES-CA `docker cp` is now best-effort and `timeout`-bounded, `scripts/setup/es_ca_cache.sh` reinstates the last pin-verified CA from `StateDirectory=` when the Docker CLI is unavailable, and a security-auditor + code-reviewer pass added pin checks in both cache verbs, a non-forgiven usage exit status, `TimeoutStartSec=`, and the removal of `RemoveIPC=true`. The purple-team pass on #550 opened five more: **#555/#557/#558 closed** in prior sessions (`stack_health.sh` finally installed on a timer with mutual cross-monitoring against `slo-metrics.service`; `.gitattributes` covers `*.service`/`*.timer`; sandboxing parity ported from `intel-refresh.service`). **#549/#554/#556/#562 landed 2026-09-07** (branch `claude/backlog-work-j9ojvi`, PR pending merge — structurally validated only, no live capture host reachable from that session): an output-derived liveness supervisor for `zeek-host-capture.service` (restarts + alerts on a stale `conn.log` while the unit reports `active`, never while `failed`); an `OnFailure=` alert dispatcher (`soc-alert-on-failure@.service`, dependent on neither Docker nor Elasticsearch) wired onto `slo-metrics.service` and `zeek-host-capture.service`, with `NTFY_TOPIC` provisioning deliberately deferred to the still-open credential-resync gate tracked in `planned_execution.md`; the capture host's own syslog shipped to Elasticsearch plus 2 new Sigma rules covering a persistently-failing SOC unit and an ES CA-fingerprint mismatch (the Linux-side counterpart to the existing Windows-only T1562.001 coverage); and `.gitattributes` extended to the 4 extensionless files (itself included) it didn't previously govern |

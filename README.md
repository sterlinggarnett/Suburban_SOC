...
| M17 | Detection Rule Coverage & Correctness | ⏸️ 30/34 closed, 4 not actionable ([milestone](https://github.com/voltron-1/Suburban_SOC/milestone/22), no actionable work left) — a project-board audit found 7 more review-follow-up issues filed without a milestone and retroactively assigned here after the original "6/8" close; 12 more closed 2026-08-29 via PRs #494–#502; #283, #333 (externally blocked/speculative) and #415, #421 (need a live Docker/Zeek/ES stack this environment doesn't have) remain |
| M18 | ECS Pipeline & Field-Mapping Integrity | ⏸️ 12/18 closed, 6 not actionable ([milestone](https://github.com/voltron-1/Suburban_SOC/milestone/23), no actionable work left) — Logstash rename/copy drift vs. what suburban-soc-ecs.yml claims, dashboard fields that don't exist on the real mapping, truncation ceilings, and index-template rollover all closed; #326 (externally blocked, needs real Windows/PowerShell telemetry), 3 review-discovered follow-ups (#396, #403, #405) each deliberately scoped out of the fix that found it, and 2 later-filed Zeek-DNS truncation follow-ups (#389, #390) remain |
...
| M25 | Repo Audit Remediation | ⏸️ 13/16 closed ([milestone](https://github.com/voltron-1/Suburban_SOC/milestone/30), created 2026-08-31, repo-audit round 2, #508–#523) — cert org-name fix, stale doc/status corrections, missing troubleshooting + playbook entries, dead workflow removal, and UIW branding cleanup all closed via PR #525/#526; #509 (branch-protection drift) and #510 (Projects v2 board confirmation) remain, both need a human with real GitHub admin/browser access this session's tool surface can't reach; #522 (wiki Watcher-era pages) in progress, 5/6 done |
...
- **Detection framework enrichment (PR #112).** The detection plane spans
  ...
  **118 Sigma rules** (`rules/sigma/`) each carry their own ATT&CK technique tag and

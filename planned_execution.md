...
human-blocked ones (M16, M17, M18, M23). Both the M25 and M26 milestone
*objects* are still `open` on GitHub and need an owner `PATCH` — see
...
3. **Two unpinned-CA control gaps, queued for a direct fix rather than a public
   ...
   this host.
4. **Close the M25 *and* M26 milestone objects** (`gh api -X PATCH
   repos/voltron-1/Suburban_SOC/milestones/30 -f state=closed` and
   `.../milestones/31 -f state=closed`) — M25 is 17/17 and M26 is 8/8, both
   with zero open issues, but both objects are still `open`. A Claude Code
   session's permission classifier blocks the call; it needs the owner.
5. **Deploy the merged M26 units to the capture host.** PR #564 merged
...
**M17 resumed again 2026-08-29** (after the M22-close session above):
...
- **#415** (build a genuinely working EXPLOITATION emulation for
  ...
  Sigma rule matches the resulting indexed document — this sandbox has no
  live Docker daemon (`docker version` succeeds for the client, fails to
  connect to `/var/run/docker.sock`) and no reachable Elasticsearch/Kibana,
  confirmed repeatedly this session (same constraint #501/#411's live-fire
  test is built to SKIP under, not fail).
- **#421** (verify whether `detect-bruteforcing`'s `SSH::Password_Guessing`
  notice actually fires on this pipeline) needs a real, paced SSH
  brute-force emulation against a target reachable over a real capture
  path (not loopback) and a live `notice.log` to inspect — the same
  missing live-stack dependency, plus the issue's own suggested fix
...
**Stage 3 (#446) was initially blocked, then unblocked and completed the
same session.** The prior session's copy of the source file
(`university_soc_starter_ruleset.rules`) had been saved only to that
session's own ephemeral scratchpad, never to this repo, and that
container no longer existed. Rather than fabricate 100 Suricata rules
with invented SIDs/domains/patterns to fill the gap, Stage 3 was left
not-started and flagged for the user to re-supply the file — which they
did, same session.

**Stage 3 (#446) implemented 2026-08-30 once the source file arrived.**
Split the 100 rules into 10 category files under `rules/suricata/`
exactly along the source's own category headers (SIDs 9000001-9000100),
every rule shipped disabled. Verified byte-for-byte fidelity to the
source before anything else (a small script diffed every uncommented
rule line + SID set against the original — 100/100 match, 0 missing, 0
extra). Wired all 10 files (+ `local.rules`) into
`configs/suricata/suricata.yaml`'s `rule-files:` list.

Landing it through the real #445 CI lane immediately paid off: the
aggregate `-T` syntax gate against the as-shipped (disabled) files
trivially passed — because Suricata's rule loader ignores `#`-commented
lines entirely and never even attempts to parse them (confirmed by hand:
a deliberately broken commented-out rule loads silently, no error). That
meant 100 rules could have shipped with genuinely broken syntax and every
existing gate would report green. Added
`suricata_rules_eval.check_syntax_including_disabled()` — strips the
leading `#` from every record and runs a real `suricata -T` against the
result — which immediately caught 8 real syntax errors: `dsize` combined
with app-layer `http.*` keywords (packet-level vs. stream-level match,
rejected; sid 9000003), `http.hostname` used where Suricata 7.0.3's real
keyword is `http.host` (sids 9000058/63/64/69), a bare `dns.query;`
selector with no attached match (sid 9000062), `ftp.command` (not a real
keyword — the real one, `ftpdata_command:stor`, needs protocol `ftp-data`
not `ftp`; sid 9000067), and `rdp.cookie` (not a real keyword in this
build — RDP is a supported app-layer protocol but exposes no
cookie-matching buffer; replaced with a plain `tcp` rule matching the
literal `Cookie: mstshash=` field, the actual cleartext RDP negotiation
string this well-known scanner fingerprint targets). Every fix verified
by hand with a real `suricata -T` run — individually, then all 8
together — before landing; each is commented in place in its `.rules`
file. Also fixed a second real bug the same pass surfaced: the syntax
gate's own `-S` flag looping (both the pytest aggregate check and
`lint.yml`'s job) broke because Suricata 7.0.3 rejects multiple `-S`
flags — simplified both to run the real production `-c
configs/suricata/suricata.yaml` path directly, which is both simpler and
more faithful (it validates exactly what `rule-files:` says to load, not
a synthetic reconstruction of it).

3 placeholders left exactly as supplied, per this repo's own
never-invent-an-IOC/domain convention: 9000013 (`univv-edu` look-alike
domain — Suburban-SOC isn't a university, no real domain to substitute),
9000065 (`STU\d{7}` student-ID format), 9000099 (`iotbotnet` placeholder
C2 domain). DLP rules 9000065/9000066 flagged for explicit owner sign-off
before enabling, per #446's own instruction not to bulk-enable them.

Reconciliation against existing Sigma coverage: re-verified all 8 of the
source's own claimed overlaps directly against the real `rules/sigma/`
files (all confirmed real) and found one the source's own list missed —
9000069 (pastebin.com) overlaps `net_zeek_ssl_cloud_storage_exfil.yml`
too, since `pastebin.com` is on that rule's own provider list. Confirmed
9000077 (outbound RDP) has no existing Sigma equivalent — a genuine new
gap, not overlap, despite the source's claimed-overlap list bundling it
with 9000071's inbound-only overlap. Decision for every overlapping pair:
keep both as independent confirmation — neither auto-triggers SOAR yet
(Stage 1's ingest is dashboard-only), so no double-trigger risk exists at
this stage to force a suppress-vs-keep call now. Full record in
`docs/detections/suricata-starter-set.md`.

Not done, disclosed: no pcap fixtures for any of the 100 rules (a
separate body of work, premature before placeholder/threshold/DLP
decisions land anyway), no `detection_filter` threshold tuning against
real traffic, ATT&CK coverage accounting still explicitly scoped out.
`pytest tests/detections/test_suricata_rules.py` — 20/20 pass (up from
17) against the real landed set; full regression suite (`tests/pipeline
tests/dashboards tests/detections/test_sigma_detections.py
tests/detections/test_threshold_rules.py tests/setup
tests/test_coverage_checklist_sync.py`) — 494 passed, 58 subtests,
unaffected; `ruff`/`mypy`/`yamllint` clean.
...
- [x] **#374 (security, priority:low) — COMPLETE, MERGED (PR #454)** —
  `configs/elasticsearch/roles/soc_admin.json` granted `"all"` on a bare
  `soc-*` wildcard, a separation-of-duties gap: the platform-admin role
  could silently rewrite/erase `soc-slo-metrics` (the SOC's own SLO
  history) or `soc-agent-health-*` (the audit-write-failure marker
  index #275/#361 depend on), with no independent record to show
  tampering happened. This issue explicitly framed the question as a
  design decision, not a prescribed fix — decided to narrow via
  Elasticsearch's negated-index-pattern role syntax
  (`-soc-slo-metrics`/`-soc-agent-health-*` appended to the existing
  `soc-*` grant), so `soc_admin` now holds zero privileges on those two
  patterns; confirmed no script/service depends on `soc_admin` alone
  reading/writing them (`slo_metrics_reader` already exists as the
  dedicated read role). Deliberately left `soc-audit-*` untouched — the
  issue didn't name it — and filed
  [#453](https://github.com/voltron-1/Suburban_SOC/issues/453)
  (unmilestoned) for the identical exposure there, found during this
  fix's security review. Code review caught a real bug in the first
  draft (a bare `assert` in a test helper, silently stripped under
  `python -O`) — fixed before merge and verified passing under both
  normal and `-O` execution. Elasticsearch's negated-pattern enforcement
...
- [x] **#296 (priority:low) — COMPLETE, MERGED (PR #490).**
  ...
  [nist.800-53.*, cis.*, nist.csf.*]`" to align with a
  `compliance_matrix.csv` — a corpus-wide grep confirmed zero of the 108
  rules in `rules/sigma/` ever carried an `nist.*`/`cis.*` tag, and
  `compliance_matrix.csv` has never existed in this repo
  (`docs/compliance_matrix.md`, a `.md` file, does). A documented control
  that was never implemented is worse than no documented control at all.
  Per the issue's own second suggested option, removed the stale,
  never-wired-up comment and replaced it with an accurate pointer to
  `docs/compliance_matrix.md` — a hand-maintained table already mapping
  specific rule filenames to specific NIST CSF/800-53/800-171/CIS control
  IDs directly, with no dependency on Sigma tags or the ES deployer.
...
Opened as "continue working the backlog docker is up". Docker *was* up — but the
premise behind the blocked list was wrong, and the session became an incident
response instead.

- **Both sensor lanes were blind, and it was not Docker.**
  - **Zeek dead five days.** `/etc/default/zeek-host-capture` still pinned
    `CAPTURE_IFACE=eth6` after WSL2 renumbered the host NICs; eth6 came back
    `DOWN` (live interface `eth4`, `192.168.1.103/24`, default route). Every
    start died with `tcpdump: eth6: That device is not up`. `Restart=always` +
    `StartLimitIntervalSec=0` made that an unbounded crash loop reported as
    `active`; at restart #123 it then **wedged** at `active (running)` with
    `docker run` hung on a container whose Zeek had already exited. Last write
    before recovery: `conn.log`, `Aug 31 20:56`.
  - **Suricata capturing zero packets.** The installed unit is the *distro*
    `suricata.service` (the repo's `suricata-host-capture.service` is not
    installed), bound to `af-packet: interface: eth0` — also `DOWN`.
    `"kernel_packets":0`, `"decoder":{"pkts":0` verbatim from `eve.json`, whose
    263 MB is almost pure stats events, which is why the lane looked alive.
    Evidence for **#443**, which correctly stays open.
  - **The monitoring died of the same cause.** `slo-metrics.service` fails
    `203/EXEC` because its `ExecStartPre=/usr/bin/docker cp …` dangles whenever
    Docker Desktop is stopped. So the lane holding
    `metric_zeek_ingest_lag_seconds()` was down for the same five days —
    coverage was **not independent**.
- **Fixed and merged: PR #551** (`2d70095`). Capture-interface preflight
  (`scripts/setup/capture_iface_preflight.sh`) exits 78 (`EX_CONFIG`) on an
  unusable pin; `RestartPreventExitStatus=78` parks the unit in `failed` where
  `systemctl is-active` reports the truth. `Restart=always` and
  `StartLimitIntervalSec=0` deliberately preserved (the Docker Desktop boot race
  needs them) and pinned by a test. 20 regression tests using a fake `ip` on
  PATH, matching the fake-`gh`/fake-`curl` convention in `tests/setup/`.
- **Review caught seven real defects before merge** (parallel `security-auditor`
  + `code-reviewer`), the sharpest three: a greedy `.*<` in the flag extraction
  bound to the *last* `<...>` on the line, so an interface alias containing
  `<...,UP>` made a DOWN interface parse as **up** (reproduced with
  `unshare -rn`); `docker run` passes the container's exit code through
  verbatim, so a Zeek-side 78 would have parked the sensor permanently; and a
  bare `var="$(...)"` in the diagnostic could exit before `exit 78` under
  `set -euo pipefail`, making the whole fix inert.
- **Two late catches of my own.** The preflight was invoked relying on the exec
  bit, which git does not carry here (`core.fileMode=false`, all
  `scripts/setup/` scripts tracked `100644`) — a fresh clone would have hit
  `Permission denied`. And the first CI run went red on `ruff`/`mypy`/
  `detections` because the test used escape sequences inside f-string
  expressions: PEP 701 syntax, fine on the local **3.12**, a SyntaxError on the
  **3.11 CI runs**. Both fixed; final CI **19/19 green**.
- **Capture restored.** Owner set `CAPTURE_IFACE=eth4`, restarted, and installed
  the updated unit. Verified `tcpdump: listening on eth4`, `NRestarts=0`,
  `conn.log` growing again.
- **New milestone M26** with two follow-ups: **#549** (output-derived liveness
  supervisor — the preflight closes the *trigger*, not the wedge; `WatchdogSec=`
  is unusable because the bash `ExecStart` cannot `sd_notify()`) and **#550**
  (`slo-metrics.service`'s Docker CLI dependency).
- Full narrative: `findings/20260905-sensor-lane-blindness.md`,
  `logs/session-20260905.md`.
...
- **#539 done (final item of the day).** `setup_branch_protection.sh`
  rewritten: REQUIRED_CHECKS = the live 9 by job name, unioned with the
  live list before the PUT so a stale copy can never weaken the policy;
  `enforce_admins: true` kept; review gate off by default behind
  `REQUIRE_REVIEW=1` (owner decision, #509); pip-audit deliberately not

...
Evidence collection is manual but relies on the automated logging of the AI Agent and the ingest-time SOAR trigger in `configs/logstash.conf` (HMAC-signed dispatch to the Hive-Mind Broker, gated on human approval via the approval queue — [#267](https://github.com/voltron-1/Suburban_SOC/issues/267) retired the separate Kibana Watcher install this used to require) to produce the required alerts and audit trails.
